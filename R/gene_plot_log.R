...
gene_plot_log = function(GeneModel, txConsensus, txReads, txHighlight,
    title, plotTopToBottom=TRUE, axisHeight= 20, consensusTrackHeight = 5,
    consensusFeatureHeight = 3, trackHeights, readHeight) {
    ...
    chrom_plot(plotDat,coord=c(start(coord), end(coord)),
        ...
        config=generate_plot_config(list(readConsensus=list(height=consensusTrackHeight), geneModel=list(height=5))),
